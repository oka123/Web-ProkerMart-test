...
function TrendBadge({ curr, prev }: { curr: number; prev: number }) {
  ...
  return (
    <span
      className={`flex items-center gap-0.5 text-xs font-semibold px-2 py-1 rounded-full ${up ? "bg-emerald-50 text-emerald-600" : "bg-red-50 text-red-600"}`}
    >
      {up ? (
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const sumHarga = (rows: any[]) =>
        rows
          .filter((r) => r.status_pesanan === "selesai")
          .reduce((s, r) => s + Number(r.total_harga), 0);
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {[0, 1, 2, 3].map((i) => (
            <div
              key={i}
              className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm animate-pulse h-32"
            />
          ))}
        </div>
      ) : (
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {recentOrders.map((order) => {
                    const s = STATUS_LABEL[order.status_pesanan] ?? {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-5 py-3.5">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded-full text-xs font-semibold ${s.color}`}
                          >
                            {s.label}
                          </span>
                        </td>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
          ...
          <div className="p-5 flex-1 flex flex-col gap-3">
            {loading ? (
              ...
              <>
                {pendingCount === 0 ? (
                  <div className="flex-1 flex flex-col items-center justify-center gap-2 text-slate-400 text-sm py-4 border border-dashed border-slate-200 rounded-xl">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-bold text-amber-900">
                          {pendingCount} Pesanan Aktif
                        </p>
                        <p className="text-xs text-amber-700">
                          Perlu konfirmasi atau pengiriman
                        </p>
                      </div>
                    </div>
                ...
                <button
                  ...
                  className="flex items-center justify-between p-4 rounded-xl bg-blue-50 border border-blue-100 hover:bg-blue-100 transition-colors text-left"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-blue-700">
                        Hubungi pembeli & lihat pesan masuk
                      </p>
                    </div>
                  </div>
