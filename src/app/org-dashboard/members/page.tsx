...
export default function MembersPage() {
  const [members, setMembers] = useState([
    { id: 1, name: "Budi Santoso", role: "Ketua", scope: "org" },
    { id: 2, name: "Siti Aminah", role: "Bendahara", scope: "org" },
    { id: 3, name: "Agus Pratama", role: "Ketua Pelaksana", scope: "p1" },
    { id: 4, name: "Rina Kumala", role: "Divisi Danus", scope: "p2" },
    { id: 5, name: "Dewi Lestari", role: "Anggota Staff", scope: "p3" },
  ]);
  ...
  const handleAddMember = (e: React.FormEvent) => {
    ...
    setNewMember({ name: "", role: "Anggota Staff", scope: "org" });
    ...
    <div className="max-w-6xl mx-auto space-y-6 relative">
      ...
      <AnimatePresence>
        {showToast && (
          ...
          >
            ...
            <span className="text-sm font-semibold">Anggota berhasil ditambahkan!</span>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-white p-6 rounded-2xl border border-slate-200 shadow-sm gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Tambah anggota, atur jabatannya, dan tugaskan ke Proker tertentu.</p>
        </div>
        <button 
        ...
        {showAddForm && (
          ...
          >
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
              <h2 className="text-md font-bold text-slate-800 mb-4">Form Anggota Baru</h2>
              <form onSubmit={handleAddMember} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                ...
                <div className="md:col-span-4 mt-2 flex justify-end">
                  <button type="submit" className="bg-primary-600 hover:bg-primary-700 text-white px-6 py-2.5 rounded-xl text-sm font-medium transition-colors flex items-center gap-2">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex gap-3 text-blue-800">
        ...
        <p className="text-sm">
          <strong>Tips:</strong> Anda bisa langsung mengubah Jabatan dan Penugasan anggota dengan mengklik opsi pada tabel di bawah. Perubahan akan tersimpan secara otomatis.
        </p>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-50 text-slate-500 font-medium border-b border-slate-200">
              <tr>
                <th className="px-6 py-4 w-1/3">Nama Anggota</th>
                <th className="px-6 py-4 w-1/4">Jabatan (Klik untuk ubah)</th>
                <th className="px-6 py-4 w-1/3">Penugasan / Proker (Klik untuk ubah)</th>
              ...
              {members.length === 0 ? (
                ...
                members.map((member) => (
                  <tr key={member.id} className="hover:bg-slate-50 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      {/* Select Jabatan Editable */}
                      <select 
                        value={member.role}
                        onChange={(e) => updateMember(member.id, 'role', e.target.value)}
                        className="border border-slate-200 bg-white hover:bg-slate-50 rounded-lg py-1.5 px-2 text-sm text-slate-700 font-medium cursor-pointer focus:ring-2 focus:ring-primary-500 outline-none w-full shadow-sm transition-all"
                      >
                        {ROLES.map(r => <option key={r} value={r}>{r}</option>)}
                      </select>
                    </td>
                    <td className="px-6 py-4">
                      {/* Select Penugasan Editable */}
                      <select 
                        value={member.scope}
                        onChange={(e) => updateMember(member.id, 'scope', e.target.value)}
                        className={`border rounded-lg py-1.5 px-2 text-sm font-semibold cursor-pointer focus:ring-2 focus:ring-primary-500 outline-none w-full shadow-sm transition-all ${
                          member.scope === 'org' 
